...
def dgp():
    ...
    L=np.random.randint(0, 5, size=(Osize+Esize, 1))
    U_cov = 1 * np.eye(2)
    X_cov = 1 * np.eye(2)
    U,X = [], []
    for i in range(Osize+Esize):
        u = np.random.multivariate_normal(
            mean=np.dot(L[i],np.array([[-1, 2]])).squeeze()
            , cov=U_cov,
            size=1)
        x = np.random.multivariate_normal(
            mean=0.5*np.dot(u,np.array([[2, 0],[0,1]])).squeeze()+0.5*np.dot(L[i],np.array([[1, 2]])).squeeze()
            , cov=X_cov, size=1)
        if i == 0:
            U = u
            X = x
    ...
    SO_treated = 1+1 + 2*np.mean(XO, axis=1) + 2 * 2 * np.mean(XO, axis=1) +np.mean(XO,axis=1) + 2*np.mean(XO,axis=1) + 2*np.mean(UO, axis=1) + noiseSO
    SO_control = 1   + 2*np.mean(XO, axis=1)                               +np.mean(XO,axis=1)                                   + 2*np.mean(UO, axis=1) + noiseSO
    SE_treated = 1+1 + 2*np.mean(XE, axis=1) + 2 * 2 * np.mean(XE, axis=1) +np.mean(XE,axis=1) + 2*np.mean(XE,axis=1) + 2*np.mean(UE, axis=1) + noiseSE
    SE_control = 1   + 2*np.mean(XE, axis=1)                               +np.mean(XE,axis=1)                                   + 2*np.mean(UE, axis=1) + noiseSE

    noiseO, noiseE = np.random.randn(Osize) * 0.7, np.random.randn(Esize) * 0.7
    YO_treated = 2 + 3 + 2*np.mean(XO, axis=1) + 8*np.mean(XO, axis=1) + 2*np.mean(XO,axis=1) + 4*np.mean(XO,axis=1) + 4*np.mean(UO, axis=1) - SO_treated +noiseO
    YO_control = 2     + 2*np.mean(XO, axis=1) +                       + 2*np.mean(XO,axis=1) +                                 + 4*np.mean(UO, axis=1) - SO_control +noiseO
    YE_treated = 2 + 3 + 2*np.mean(XE, axis=1) + 8*np.mean(XE, axis=1) + 2*np.mean(XE,axis=1) + 4*np.mean(XE,axis=1) + 4*np.mean(UE, axis=1) - SE_treated +noiseE
    YE_control = 2     + 2*np.mean(XE, axis=1) +                       + 2*np.mean(XE,axis=1) +                                 + 4*np.mean(UE, axis=1) - SE_control +noiseE
