...
def dgp():
    ...
    YO_treated = 4 + np.mean(XO, axis=1) + np.mean(XO, axis=1) + SO_treated + 6 * np.mean(UO, axis=1)+noiseO
    YO_control =     np.mean(XO, axis=1)                       + SO_control +6 * np.mean(UO, axis=1)+ noiseO
    YE_treated = 4 + np.mean(XE, axis=1) +np.mean(XE, axis=1) + SE_treated + 6 * np.mean(UE, axis=1)+ noiseE
    YE_control =     np.mean(XE, axis=1)                      + SE_control + 6* np.mean(UE, axis=1)+noiseE
