...
def dgp():
    ...
    for i in range(Osize+Esize):
        ...
        x = np.random.multivariate_normal(
            mean=np.dot(u,np.array([[2, 0],[0,1]])).squeeze()
            , cov=X_cov, size=1)
        if i == 0:
            U = u
            X = x
    ...
    YO_treated = 4 + np.mean(XO, axis=1) + np.mean(XO, axis=1) + SO_treated + 1 * np.mean(UO, axis=1)+noiseO
    YO_control =     np.mean(XO, axis=1)                       + SO_control +1 * np.mean(UO, axis=1)+ noiseO
    YE_treated = 4 + np.mean(XE, axis=1) +np.mean(XE, axis=1) + SE_treated + 1 * np.mean(UE, axis=1)+ noiseE
    YE_control =     np.mean(XE, axis=1)                      + SE_control + 1* np.mean(UE, axis=1)+noiseE
