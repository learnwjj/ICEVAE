...
from data.Synthetic_data.Complete_Gragh_Dif_Beta_Z2W_Z2Y import dgp
import pandas as pd
...
def icevae_Dif_Beta_exp(epochs=200,hidden_dim=32,batch_size=50,lr=1e-3,latent_dim=10,DifbetaZ=1):
    ...
    for i in range(num_experiments):
        ...
        U_total, Z_total, X_total, W_total, S_total, Y_total, G_total, \
            ...
            Ite_List, Ite_List_exp=dgp(betaZ=DifbetaZ)
...
if __name__ == '__main__':
    ...
    PEHE_mean, PEHE_std, MSEmean, MSEstd = icevae_Dif_Beta_exp(epochs=200, hidden_dim=64,batch_size=100, lr=1e-4, latent_dim=10,DifbetaZ=1)

    PEHE_mean, PEHE_std, MSEmean, MSEstd = icevae_Dif_Beta_exp(epochs=200, hidden_dim=64, batch_size=100, lr=1e-4,latent_dim=10, DifbetaZ=1.5)

    PEHE_mean, PEHE_std, MSEmean, MSEstd = icevae_Dif_Beta_exp(epochs=200, hidden_dim=64, batch_size=32, lr=1e-4,latent_dim=10, DifbetaZ=3)

    PEHE_mean, PEHE_std, MSEmean, MSEstd = icevae_Dif_Beta_exp(epochs=200, hidden_dim=128, batch_size=50, lr=1e-3,latent_dim=10, DifbetaZ=4.5)

    PEHE_mean, PEHE_std, MSEmean, MSEstd = icevae_Dif_Beta_exp(epochs=200, hidden_dim=128, batch_size=50, lr=1e-3,latent_dim=10, DifbetaZ=5)
